...
- **Implemented**: `010-exclude-and-require-baseline` (2026-09-20)
  - **Summary**: Escape hatch and strictness controls for `0.4.0`, designed for AI agents as the primary consumer. `--exclude PATTERN` (repeatable, on `check`/`init`/`accept`/`refresh`) and `<root>/.pydocsync.json` (`exclude`, `default_excludes`, `require_baseline`; JSON because Python 3.10 has no stdlib TOML) using a strict gitignore-style subset (`pydocsync/patterns.py`; invalid or unsupported syntax exits 2, quoting the pattern). Exclusions are always visible (`excluded by rules: N path(s)`, warning for patterns that match nothing); an unusable config exits 2 instead of meaning "no exclusions". `node_modules`/`site-packages` are always skipped; `--no-default-excludes` scans `build`/`tests`/... (`DEFAULT_IGNORED_DIRS` stays a superset). `check --require-baseline` reports `BASELINE_MISSING` (exit 2). `accept --file` on an excluded file exits 1 naming the rule; suggested `accept`/`refresh` commands carry `--no-default-excludes` when it was used. Shipped `.pre-commit-hooks.yaml` (verified by running its entry command, not under pre-commit itself). 271/271 tests passing (118 new for spec 010; 116 of them fail against the spec-009 commit, the other 2 are compatibility guards).
  - **Spec & Plan**: [`specs/010-exclude-and-require-baseline/spec.md`](specs/010-exclude-and-require-baseline/spec.md) | [`plan.md`](specs/010-exclude-and-require-baseline/plan.md) | [`tasks.md`](specs/010-exclude-and-require-baseline/tasks.md) | [`convergence_report.md`](specs/010-exclude-and-require-baseline/convergence_report.md)
  - **Tests**: `tests/test_exclusions_and_require_baseline.py`, full suite `tests/` (271/271 passed).

- **Implemented**: `009-silent-false-pass-hardening` (2026-09-20)
  - **Summary**: Eliminated silent "All symbols synchronized" results for `0.4.0`. Corrupt/unreadable/unsupported baselines and unreadable/unparseable Python files are reported as structured problems (exit 2, all reported in one run, drift printed alongside; `SyncResult.problems` for the API). `accept` refuses ambiguous names and gains `--file`; `init` protects records that `check` flags (`--force --reason`, `--dry-run`); same-named definitions in one file are tracked per definition (`name`, `name#2`); stale baselines (docs updated, baseline not refreshed) are reported and recorded via the new `refresh` command (`check --fail-on-stale` for CI). Added `PyDocSyncError` hierarchy (not a `ValueError`) replacing the broad `ValueError` catch, and a coverage line on successful checks. Found and fixed a wider silent skip: files containing a docstring-only class were skipped entirely by the extractor. 153/153 tests passing (59 new; 52 of them fail on the v0.3.0 source).
  - **Spec & Plan**: [`specs/009-silent-false-pass-hardening/spec.md`](specs/009-silent-false-pass-hardening/spec.md) | [`plan.md`](specs/009-silent-false-pass-hardening/plan.md) | [`tasks.md`](specs/009-silent-false-pass-hardening/tasks.md) | [`convergence_report.md`](specs/009-silent-false-pass-hardening/convergence_report.md)
  - **Tests**: `tests/test_silent_false_pass_regressions.py` (59 new), full suite `tests/` (153/153 passed).
